...
inline void ymLog(uint32 const    Verbosity,
                  SStr_T const    Format,
                  Args_T const... Args);
...
{
public:
   explicit TextLogger(SStr_T const Filename);
...
private:
   ...
   template <typename... Args_T>
   void printf_Helper(SStr_T const    Format,
                      Args_T const... Args);

   template <typename... Args_T>
   void printfError(SStr_T const    Format,
                    Args_T const... Args);
 ...
 */
template <typename... Args_T>
void TextLogger::printfError(SStr_T const    Format,
                             Args_T const... Args)
{
   if (isGlobalInstance())
 ...
 */
template <typename... Args_T>
void TextLogger::printf_Helper(SStr_T const    Format,
                               Args_T const... Args)
{
   _availableSem.wait();
