...
public:
   ...
   explicit constexpr Point(double x_,
                            double y_);

   // default the big three
   inline ~Point(void) = default;
   constexpr Point              (Point const & Rhs) = default;
   constexpr Point & operator = (Point const & Rhs) = default;

   constexpr void neg(void); // negate

   // point-wise
   constexpr void pls(Point const & Rhs);
   constexpr void mns(Point const & Rhs);
   constexpr void tms(Point const & Rhs);
   constexpr void dvd(Point const & Rhs);
   constexpr void pow(Point const & Rhs);

   // constexpr Point operator - (void) const;
   ...
   float64 x; // x-coordinate (real)
   float64 y; // y-coordinate (imaginary)
}; // Point
