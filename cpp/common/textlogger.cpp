...
#define HOSO_PRINT_TO_SCREEN

/**
 * 
 */
hoso::TextLogger::TextLogger(str  const Filename,
 ...
 */
auto hoso::TextLogger::getGlobalInstancePtr(void) -> TextLogger *
{
   ...
   if (!s_globalInstance.isOpen())
   {
      { // finish populating name
      ...
      #else
         auto * const timeinfo_Ptr = std::localtime(&t);
         hosoThrow(timeinfo_Ptr, "Failure to get local time!");
         auto timeinfo = *timeinfo_Ptr;
      #endif // _WIN32
         ...
         hosoThrow(NBytesWritten > 0, "Failure to store datetime!");
      }

      auto const DidOpen = s_globalInstance.open();

      hosoAssert(DidOpen, "%s could not open!", globalName);
   }
 ...
 *
 */
hoso::TextLogger::~TextLogger(void)
{
   close();
}
 ...
 *
 */
bool hoso::TextLogger::open(void)
{
   std::scoped_lock const Lock(_openingGuard);

   if (Logger::open(getName()))
 ...
 *
 */
void hoso::TextLogger::close(void)
{
   bool       expected = true;
 ...
 *
 */
void hoso::TextLogger::enableClients(uint64 const Clients_mask)
{
   hosoThrow(Clients_mask >= 4, "Clients mask must be >= 4 (last 2 bits reserved for verbosity)!");
 ...
 *
 */
void hoso::TextLogger::disableClients(uint64 const Clients_mask)
{
   hosoThrow(Clients_mask >= 4, "Clients mask must be >= 4 (last 2 bits reserved for verbosity)!");
 ...
 *
 */
auto hoso::TextLogger::getVerbosityCap(void) const -> uint64
{
 ...
 *
 */
void hoso::TextLogger::setVerbosityCap(uint64 const VerbosityCap)
{
   hosoThrow(VerbosityCap <= 3, "Verbosity cap must be <= 3!");
 ...
 */
bool hoso::TextLogger::willPrint(uint64 const Verbosities)
{
 ...
 *
 */
void hoso::TextLogger::writeMessagesToFile(void)
{
   do
   { // wait for messages to print while logger is still active
      ...
      { // wait until THIS message is in the buffer
         ...
         _messagesSem.wait(); // waits until A message is in the buffer
      }

      auto const * const Read_Ptr = _buffer + (_readPos * getMaxMessageSize_bytes());

      auto const NCharsWritten = std::fprintf(getOutfilePtr(), "%s\n", Read_Ptr);
   ...
   #if defined(HOSO_PRINT_TO_SCREEN)
      if (isGlobalInstance())
      {
         std::fprintf(stdout, "%s\n", Read_Ptr);
      }
   #endif // HOSO_PRINT_TO_SCREEN
 ...
 */
auto hoso::TextLogger::populateFormattedTime(char * const write_Ptr) const -> uint32
{
   auto const ElapsedTime_us  = hosoGetGlobalTime<std::micro>();
