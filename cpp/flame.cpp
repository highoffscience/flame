...
int main(const int argc, const char* const* const argv)
{
   ...
   for (uint y = 0; y < Height; ++y)
   {
      for (uint x = 0; x < Width; ++x)
      {
         ...
         auto const R = static_cast<uint8>((pxl.r * 255.0_f) /*+ 0.5*/);
         auto const G = static_cast<uint8>((pxl.g * 255.0_f) /*+ 0.5*/);
         auto const B = static_cast<uint8>((pxl.b * 255.0_f) /*+ 0.5*/);
         auto const A = static_cast<uint8>((pxl.a * 255.0_f) /*+ 0.5*/);
